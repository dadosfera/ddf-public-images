...
log_env_section_001 "${SCRIPT_OUTPUT} Find last chrome version"
# install necessary dependencies
sudo apt-get update --q -y
sudo apt-get install -q -y --no-install-recommends libasound2 libdrm2 libgbm1 libu2f-udev libvulkan1 xdg-utils

log_env_section_001 "${SCRIPT_OUTPUT} Install chrome dependencies"
...
log_env_section_001 "${SCRIPT_OUTPUT} Download current chrome"
...
if [ -z "$chrome_path" ]; then
    log_env_section_001 "${SCRIPT_OUTPUT} ERROR: Chrome binary not found"
    exit 1
else
    ...
    log_env_section_001 "${SCRIPT_OUTPUT} Download current chrome driver"
    ...
    curl -o "chromedriver.zip" "https://chromedriver.storage.googleapis.com/$CHROMEDRIVER_VERSION/chromedriver_linux64.zip">>"$stdall_log_env_section_001_file" 2>&1 || {
        log_env_section_001 "${SCRIPT_OUTPUT} ERROR: Failed to download ChromeDriver"
        exit 1
    }

    log_env_section_001 "${SCRIPT_OUTPUT} Checking chrome dir"
    ...
    log_env_section_001 "${SCRIPT_OUTPUT} Unzip chrome to ${CHROMEDRIVER_PATH}"
    # Unzip ChromeDriver handling
    unzip chromedriver.zip -d "$CHROMEDRIVER_DIR" >>"$stdall_log_env_section_001_file" 2>&1 || {
        log_env_section_001 "${SCRIPT_OUTPUT} ERROR: Failed to unzip ChromeDriver"
        exit 1
    }
    ...
    sudo chmod +x "$CHROMEDRIVER_PATH" || {
        log_env_section_001 "${SCRIPT_OUTPUT} ERROR: Failed to set executable permissions for ChromeDriver"
        exit 1
    }

    # Add ChromeDriver to the PATH in a persistent way
    log_env_section_001 "${SCRIPT_OUTPUT} export PATH=\$PATH:$CHROMEDRIVER_DIR" >> ~/.bashrc
    log_env_section_001 "${SCRIPT_OUTPUT} INFO: Chrome and ChromeDriver installed successfully!"
    log_env_section_001 "${SCRIPT_OUTPUT} Installed Chrome version: $CHROME_VERSION"
    log_env_section_001 "${SCRIPT_OUTPUT} Installed ChromeDriver version: $CHROMEDRIVER_VERSION"
    export PATH=$PATH:$CHROMEDRIVER_DIR
fi

log_env_section_001 "${SCRIPT_OUTPUT} Finished Chrome and ChromeDriver installation."
